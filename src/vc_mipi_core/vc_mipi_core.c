...
__u32 vc_core_calculate_max_frame_rate(struct vc_cam *cam, __u8 num_lanes,
				       __u8 format, __u8 binning, __u32 height);
...
static void vc_core_print_mode(struct vc_cam *cam)
{
	...
	if (ctrl->flags & FLAG_INCREASE_FRAME_RATE) {
		...
		vc_notice(
			dev,
			"| lanes | format | binning | exposure   | framerate |\n");
		vc_notice(
			dev,
			"|       |        | mode    | max [us]   | max [mHz] |\n");
		vc_notice(
		...
		while (index < MAX_VC_MODES &&
			...
			__u32 max_frame_rate = vc_core_calculate_max_frame_rate(
				cam, num_lanes, format, binning, height);

			vc_core_print_format(format, sformat);
			vc_notice(
				dev,
				"|     %1d | %s  |       %1d | %10d | %9d |\n",
				num_lanes, sformat, binning, max_exposure,
				max_frame_rate);
			index++;
		}
		vc_notice(
...
static int vc_core_fmt_to_str(__u32 code, char *buf)
{
	...
	case MEDIA_BUS_FMT_Y10_1X10:
		sprintf(buf, "Y10_1X10     ");
		break;
	case MEDIA_BUS_FMT_Y12_1X12:
		sprintf(buf, "Y12_1X12     ");
		break;
	case MEDIA_BUS_FMT_Y14_1X14:
		sprintf(buf, "Y14_1X14     ");
		break;
	case MEDIA_BUS_FMT_SRGGB8_1X8:
		sprintf(buf, "SRGGB8_1X8   ");
		break;
	case MEDIA_BUS_FMT_SRGGB10_1X10:
		sprintf(buf, "SRGGB10_1X10 ");
		break;
	case MEDIA_BUS_FMT_SRGGB12_1X12:
		sprintf(buf, "SRGGB12_1X12 ");
		break;
	case MEDIA_BUS_FMT_SRGGB14_1X14:
		sprintf(buf, "SRGGB14_1X14 ");
		break;
	case MEDIA_BUS_FMT_SGBRG8_1X8:
		sprintf(buf, "SGBRG8_1X8   ");
		break;
	case MEDIA_BUS_FMT_SGBRG10_1X10:
		sprintf(buf, "SGBRG10_1X10 ");
		break;
	case MEDIA_BUS_FMT_SGBRG12_1X12:
		sprintf(buf, "SGBRG12_1X12 ");
		break;
	case MEDIA_BUS_FMT_SGBRG14_1X14:
		sprintf(buf, "SGBRG14_1X14 ");
		break;
	default:
		return -EINVAL;
	}
...
void vc_core_update_mbus_codes(struct vc_cam *cam)
{
	...
	for (modeIx = 0; modeIx < desc->num_modes; modeIx++) {
		...
		__u32 code = vc_core_format_to_mbus_code(mode->format, is_color,
							 is_bgrg);
		vc_dbg(dev, "%s(): Checking mode %u (code: 0x%04x)\n",
		       __FUNCTION__, modeIx, code);

		for (codeIx = 0; codeIx < ARRAY_SIZE(ctrl->mbus_codes);
		     codeIx++) {
	...
	vc_dbg(dev,
	       "%s(): MBUS codes (0x%04x, 0x%04x, 0x%04x, 0x%04x, 0x%04x)\n",
	       __FUNCTION__, ctrl->mbus_codes[0], ctrl->mbus_codes[1],
	       ctrl->mbus_codes[2], ctrl->mbus_codes[3], ctrl->mbus_codes[4]);
}
...
int vc_core_live_roi(struct vc_cam *cam, __s32 data)
{
	...
	vc_notice(dev, "%s(): binning_mode: %u, left: %u, top: %u\n",
		  __FUNCTION__, binning_mode, left, top);
...
__u32 vc_core_get_optimized_vmax(struct vc_cam *cam, __u8 num_lanes,
				 __u8 format, __u8 binning_mode, __u32 height)
{
	struct vc_ctrl *ctrl = &cam->ctrl;
	...
	vc_dbg(dev, "%s(): vmax_def: %u, v_factor: %u, height: %u/%u\n",
	       __FUNCTION__, vmax_def, binning->v_factor, height,
	       ctrl->frame.height);
				       ...
				       __u8 format, __u8 binning, __u32 height)
{
	...
	__u32 vmax = vc_core_get_optimized_vmax(cam, num_lanes, format, binning,
						height);
...
int vc_sen_set_gain(struct vc_cam *cam, __u64 gain, bool unit_is_mdB)
{
	...
	case GAIN_LINEAR:
		...
		again = ((1000000 * (__u64)ctrl->again.max) /
			 ctrl->again.max_mdB) *
			again_mdB / 1000000;
	...
	case GAIN_LOGARITHMIC:
		...
		again = (1000 * ctrl->again.c1 - again_times * ctrl->again.c0) /
			1000;
		vc_dbg(dev, "%s(): GAIN_LOG %u mdB -> %u times\n", __FUNCTION__,
		       again_mdB, again_times);
		break;

	case GAIN_RECIPROCAL:
		again_times = vc_core_mdB_to_times(again_mdB);
		again = ctrl->again.c1 -
			1000 * (__u64)ctrl->again.c0 / again_times;
	...
	case GAIN_FRACTIONAL:
		again_times = vc_core_mdB_to_times(again_mdB);
		again_fraction =
			((again_times - (again_times / 1000) * 1000) * 16) /
			1000;
		again = (again_times / 1000 << 4) + again_fraction;
		vc_dbg(dev,
		       "%s(): GAIN_FRA %u mdB -> %u times (%u.%u) => 0x%02x\n",
		       __FUNCTION__, again_mdB, again_times, again_times / 1000,
		       again_fraction, again);
		break;
	}
	...
	default:
		...
		dgain = (dgain_times / 1000 << 8) +
			((dgain_times - (dgain_times / 1000) * 1000) * 256) /
				1000;
		break;
	}
	...
	vc_dbg(dev, "%s(): gain:%llu %s, again:%ux/%u/%u, dgain:%ux/%u/%u\n",
	       __FUNCTION__, gain, unit_is_mdB ? "mdB" : "times", again_times,
	       again, ctrl->again.max, dgain_times, dgain, ctrl->dgain.max);
	vc_notice(dev, "%s(): Set sensor gain: %u mdB (exposure: %u us)\n",
		  __FUNCTION__, gain_mdB, cam->state.exposure);

	ret |= i2c_write_reg2(dev, client, &ctrl->csr.sen.again, again,
			      __FUNCTION__);
	ret |= i2c_write_reg2(dev, client, &ctrl->csr.sen.dgain, dgain,
			      __FUNCTION__);
	if (ret) {
...
int vc_sen_set_blacklevel(struct vc_cam *cam, __u32 blacklevel_rel)
{
	...
	vc_notice(dev, "%s(): Set sensor black level: %u (%u/%u)\n",
		  __FUNCTION__, blacklevel_rel, blacklevel_abs, blacklevel_max);
					 ...
					 __u8 format, __u8 binning)
{
	...
	for (index = 0; index <= MAX_VC_MODES; index++) {
		...
		if (mode->num_lanes == num_lanes && mode->format == format &&
		    (binning_index == ctrl->mode[index].binning)) {
			return ((__u64)mode->hmax * 1000000000) /
			       ctrl->clk_pixel;
		}
	}
...
static void vc_core_calculate_vmax(struct vc_cam *cam, __u32 period_1H_ns)
{
	...
	state->vmax = vc_core_get_optimized_vmax(cam, num_lanes, format,
						 binning, height);
...
static void vc_calculate_trig_exposure(struct vc_cam *cam, __u32 exposure_us)
{
	...
	min_frametime_us =
		((__u64)retrigger_min * 1000000) / ctrl->clk_ext_trigger;
	...
	state->retrigger_cnt =
		((__u64)frametime_us * ctrl->clk_ext_trigger) / 1000000;
	// NOTE: Check this for different cameras.
	// if (state->retrigger_cnt < 3240) {
	// 	state->retrigger_cnt = 3240;
	// }
	state->exposure_cnt =
		((__u64)exposure_us * ctrl->clk_ext_trigger) / 1000000;
}
...
int vc_sen_set_exposure(struct vc_cam *cam, int exposure_us)
{
	...
	vc_notice(dev, "%s(): Set sensor exposure: %u us (gain: %u mdB)\n",
		  __FUNCTION__, exposure_us, cam->state.gain);
	...
	if (ctrl->flags & FLAG_EXPOSURE_SONY ||
		...
		__u32 duration =
			(((__u64)exposure_us) * ctrl->flash_factor) / 1000000;
