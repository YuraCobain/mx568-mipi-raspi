...
#define MODE(index, lanes, format, binning, _hmax, vmax_min, vmax_max,         \
	     vmax_def, blacklevel_max, blacklevel_def, _retrigger_min)         \
	ctrl->mode[index] = (vc_mode){                                         \
		lanes,                                                         \
		format,                                                        \
		binning,                                                       \
		.hmax = _hmax,                                                 \
...
#define BINNING_MODE_REGS(_mode, ...)                                        \
	if (MAX_VC_MODES > _mode) {                                          \
		do {                                                         \
			const struct vc_reg _binning_mode_regs[] = {         \
				__VA_ARGS__                                  \
			};                                                   \
			int vrs = 0;                                         \
			vrs = sizeof(_binning_mode_regs) / sizeof(vc_reg);   \
			if (MAX_BINNING_MODE_REGS > vrs) {                   \
				memcpy(&ctrl->mode[_mode].binning_mode_regs, \
				       _binning_mode_regs,                   \
				       sizeof(_binning_mode_regs));          \
			}                                                    \
		} while (0);                                                 \
	}
...
{
	ctrl->exposure =
		(vc_control){ .min = 1, .max = 100000000, .def = 10000 };
...
{
	ctrl->csr.sen.vmax =
		(vc_csr4){ .l = 0x7004, .m = 0x7005, .h = 0x7006, .u = 0x0000 };
	ctrl->csr.sen.hmax =
		(vc_csr4){ .l = 0x7002, .m = 0x7003, .h = 0x0000, .u = 0x0000 };
...
static void vc_init_ctrl_imx252_base(struct vc_ctrl *ctrl, struct vc_desc *desc)
{
	...
	ctrl->csr.sen.vmax =
		(vc_csr4){ .l = 0x0210, .m = 0x0211, .h = 0x0212, .u = 0x0000 };
	ctrl->csr.sen.hmax =
		(vc_csr4){ .l = 0x0214, .m = 0x0215, .h = 0x0000, .u = 0x0000 };
...
static void vc_init_ctrl_imx290_base(struct vc_ctrl *ctrl, struct vc_desc *desc)
{
	...
	ctrl->csr.sen.vmax =
		(vc_csr4){ .l = 0x3018, .m = 0x3019, .h = 0x301A, .u = 0x0000 };
...
static void vc_init_ctrl_imx296_base(struct vc_ctrl *ctrl, struct vc_desc *desc)
{
	...
	ctrl->csr.sen.vmax =
		(vc_csr4){ .l = 0x3010, .m = 0x3011, .h = 0x3012, .u = 0x0000 };
...
static void vc_init_ctrl_imx335(struct vc_ctrl *ctrl, struct vc_desc *desc)
{
	...
	ctrl->csr.sen.vmax =
		(vc_csr4){ .l = 0x3030, .m = 0x3031, .h = 0x3032, .u = 0x0000 };
	ctrl->csr.sen.mode_standby = 0x01;
	ctrl->csr.sen.mode_operating = 0x00;
...
static void vc_init_ctrl_imx412(struct vc_ctrl *ctrl, struct vc_desc *desc)
{
	...
	ctrl->csr.sen.vmax =
		(vc_csr4){ .l = 0x0341, .m = 0x0340, .h = 0x0000, .u = 0x0000 };
	ctrl->csr.sen.shs =
		(vc_csr4){ .l = 0x0203, .m = 0x0202, .h = 0x0000, .u = 0x0000 };
	...
	BINNING_START(ctrl->binnings[0], 0, 0){ IMX412_BINNING_MODE,
						IMX412_BINNING_MODE_DISABLE },
		{ IMX412_BINNING_TYPE, 0x11 },
		{ IMX412_BLACKLEVEL_ENABLE,
		  0x01 } BINNING_END(ctrl->binnings[0])
			BINNING_START(ctrl->binnings[1], 1,
				      2){ IMX412_BINNING_MODE,
					  IMX412_BINNING_MODE_ENABLE },
		{ IMX412_BINNING_WEIGHTING, IMX412_BINNING_WEIGHTING_SUM },
		{ IMX412_BINNING_TYPE, 0x12 },
		{ IMX412_BLACKLEVEL_ENABLE,
		  0x01 } BINNING_END(ctrl->binnings[1])
			BINNING_START(ctrl->binnings[2], 2,
				      2){ IMX412_BINNING_MODE,
					  IMX412_BINNING_MODE_ENABLE },
		{ IMX412_BINNING_WEIGHTING, IMX412_BINNING_WEIGHTING_SUM },
		{ IMX412_BINNING_TYPE, 0x22 },
		{ IMX412_BLACKLEVEL_ENABLE,
		  0x01 } BINNING_END(ctrl->binnings[2])
			BINNING_START(ctrl->binnings[3], 4,
				      2){ IMX412_BINNING_MODE,
					  IMX412_BINNING_MODE_ENABLE },
		{ IMX412_BINNING_WEIGHTING, IMX412_BINNING_WEIGHTING_SUM },
		{ IMX412_BINNING_TYPE, 0x42 },
		{ IMX412_BLACKLEVEL_ENABLE,
		  0x01 } BINNING_END(ctrl->binnings[3])
			BINNING_START(ctrl->binnings[4], 8,
				      2){ IMX412_BINNING_MODE,
					  IMX412_BINNING_MODE_ENABLE },
		{ IMX412_BINNING_WEIGHTING, IMX412_BINNING_WEIGHTING_SUM },
		{ IMX412_BINNING_TYPE, 0x82 },
		{ IMX412_BLACKLEVEL_ENABLE,
		  0x01 } BINNING_END(ctrl->binnings[4])
			BINNING_START(ctrl->binnings[5], 16,
				      2){ IMX412_BINNING_MODE,
					  IMX412_BINNING_MODE_ENABLE },
		{ IMX412_BINNING_WEIGHTING, IMX412_BINNING_WEIGHTING_SUM },
		...
		{ IMX412_BLACKLEVEL_ENABLE,
		  0x01 } BINNING_END(ctrl->binnings[5])
...
static void vc_init_ctrl_imx415(struct vc_ctrl *ctrl, struct vc_desc *desc)
{
	...
	ctrl->csr.sen.vmax =
		(vc_csr4){ .l = 0x3024, .m = 0x3025, .h = 0x3026, .u = 0x0000 };
	ctrl->csr.sen.mode_standby = 0x01;
	ctrl->csr.sen.mode_operating = 0x00;
...
static void vc_init_ctrl_imx565(struct vc_ctrl *ctrl, struct vc_desc *desc)
{
	...
	ctrl->csr.sen.vmax =
		(vc_csr4){ .l = 0x30d4, .m = 0x30d5, .h = 0x30d6, .u = 0x0000 };
	ctrl->csr.sen.hmax =
		(vc_csr4){ .l = 0x30d8, .m = 0x30d9, .h = 0x0000, .u = 0x0000 };
	...
	MODE(1, 2, FORMAT_RAW10, 0, 1328, 16, 0xffffff, 0xc2a, 1023, 60,
	     2990142)
	MODE(2, 2, FORMAT_RAW12, 0, 1586, 14, 0xffffff, 0xc26, 4095, 240,
	     3568752)
	...
	MODE(5, 4, FORMAT_RAW12, 0, 812, 22, 0xffffff, 0xc34, 4095, 240,
	     1833030)
	...
	MODE(11, 4, FORMAT_RAW12, 1, 425, 40, 0xffffff, 0x654, 4095, 240,
	     493884)

	// Special registers for binning mode
	BINNING_MODE_REGS(6, { IMX56X_GMRWT, 0x04 }, { IMX56X_GMTWT, 0x1c },
			  { IMX56X_GAINDLY, 0x04 }, { IMX56X_GSDLY, 0x0c });
	BINNING_MODE_REGS(7, { IMX56X_GMRWT, 0x04 }, { IMX56X_GMTWT, 0x18 },
			  { IMX56X_GAINDLY, 0x04 }, { IMX56X_GSDLY, 0x0c });
	BINNING_MODE_REGS(8, { IMX56X_GMRWT, 0x04 }, { IMX56X_GMTWT, 0x14 },
			  { IMX56X_GAINDLY, 0x04 }, { IMX56X_GSDLY, 0x08 });
	BINNING_MODE_REGS(9, { IMX56X_GMRWT, 0x08 }, { IMX56X_GMTWT, 0x30 },
			  { IMX56X_GAINDLY, 0x04 }, { IMX56X_GSDLY, 0x18 });
	BINNING_MODE_REGS(10, { IMX56X_GMRWT, 0x08 }, { IMX56X_GMTWT, 0x28 },
			  { IMX56X_GAINDLY, 0x04 }, { IMX56X_GSDLY, 0x14 });
	BINNING_MODE_REGS(11, { IMX56X_GMRWT, 0x08 }, { IMX56X_GMTWT, 0x24 },
			  { IMX56X_GAINDLY, 0x04 }, { IMX56X_GSDLY, 0x10 });

	BINNING_START(ctrl->binnings[0], 0, 0){ IMX56X_HV_MODE,
						IMX56X_BINNING_MODE_DISABLE },
		{ IMX56X_EAV_SELECT,
		  IMX56X_EAV_SELECT_VALUE } BINNING_END(ctrl->binnings[0])

			BINNING_START(ctrl->binnings[1], 2,
				      2){ IMX56X_HV_MODE,
					  IMX56X_BINNING_MODE_ENABLE },
		{ IMX56X_EAV_SELECT,
		  IMX56X_EAV_SELECT_VALUE } BINNING_END(ctrl->binnings[1])
...
static void vc_init_ctrl_imx566(struct vc_ctrl *ctrl, struct vc_desc *desc)
{
	...
	MODE(11, 4, FORMAT_RAW12, 1, 307, 52, 0xffffff, 0x614, 4095, 240,
	     336690)

	// Special registers for binning mode
	BINNING_MODE_REGS(6, { IMX56X_GMRWT, 0x08 }, { IMX56X_GMTWT, 0x24 },
			  { IMX56X_GAINDLY, 0x04 }, { IMX56X_GSDLY, 0x10 });
	BINNING_MODE_REGS(7, { IMX56X_GMRWT, 0x04 }, { IMX56X_GMTWT, 0x20 },
			  { IMX56X_GAINDLY, 0x04 }, { IMX56X_GSDLY, 0x10 });
	BINNING_MODE_REGS(8, { IMX56X_GMRWT, 0x04 }, { IMX56X_GMTWT, 0x1c },
			  { IMX56X_GAINDLY, 0x04 }, { IMX56X_GSDLY, 0x0c });
	BINNING_MODE_REGS(9, { IMX56X_GMRWT, 0x0c }, { IMX56X_GMTWT, 0x44 },
			  { IMX56X_GAINDLY, 0x04 }, { IMX56X_GSDLY, 0x1c });
	BINNING_MODE_REGS(10, { IMX56X_GMRWT, 0x08 }, { IMX56X_GMTWT, 0x38 },
			  { IMX56X_GAINDLY, 0x04 }, { IMX56X_GSDLY, 0x18 });
	BINNING_MODE_REGS(11, { IMX56X_GMRWT, 0x08 }, { IMX56X_GMTWT, 0x30 },
			  { IMX56X_GAINDLY, 0x04 }, { IMX56X_GSDLY, 0x14 });
...
static void vc_init_ctrl_imx567(struct vc_ctrl *ctrl, struct vc_desc *desc)
{
	...
	MODE(2, 2, FORMAT_RAW12, 0, 965, 20, 0xffffff, 0x880, 4095, 240,
	     1514484)
	...
	MODE(11, 4, FORMAT_RAW12, 1, 271, 60, 0xffffff, 0x498, 4095, 240,
	     224640)

	// Special registers for binning mode
	BINNING_MODE_REGS(6, { IMX56X_GMRWT, 0x08 }, { IMX56X_GMTWT, 0x2c },
			  { IMX56X_GAINDLY, 0x04 }, { IMX56X_GSDLY, 0x14 });
	BINNING_MODE_REGS(7, { IMX56X_GMRWT, 0x08 }, { IMX56X_GMTWT, 0x24 },
			  { IMX56X_GAINDLY, 0x04 }, { IMX56X_GSDLY, 0x10 });
	BINNING_MODE_REGS(8, { IMX56X_GMRWT, 0x04 }, { IMX56X_GMTWT, 0x20 },
			  { IMX56X_GAINDLY, 0x04 }, { IMX56X_GSDLY, 0x10 });
	BINNING_MODE_REGS(9, { IMX56X_GMRWT, 0x0c }, { IMX56X_GMTWT, 0x4c },
			  { IMX56X_GAINDLY, 0x04 }, { IMX56X_GSDLY, 0x20 });
	BINNING_MODE_REGS(10, { IMX56X_GMRWT, 0x0c }, { IMX56X_GMTWT, 0x40 },
			  { IMX56X_GAINDLY, 0x04 }, { IMX56X_GSDLY, 0x1c });
	BINNING_MODE_REGS(11, { IMX56X_GMRWT, 0x08 }, { IMX56X_GMTWT, 0x38 },
			  { IMX56X_GAINDLY, 0x04 }, { IMX56X_GSDLY, 0x18 });
...
static void vc_init_ctrl_ov7251(struct vc_ctrl *ctrl, struct vc_desc *desc)
{
	...
	ctrl->csr.sen.flash_duration =
		(vc_csr4){ .l = 0x3b8f, .m = 0x3b8e, .h = 0x3b8d, .u = 0x3b8c };
	ctrl->csr.sen.flash_offset =
		(vc_csr4){ .l = 0x3b8b, .m = 0x3b8a, .h = 0x3b89, .u = 0x3b88 };
	ctrl->csr.sen.vmax =
		(vc_csr4){ .l = 0x380f, .m = 0x380e, .h = 0x0000, .u = 0x0000 };
...
static void vc_init_ctrl_ov9281(struct vc_ctrl *ctrl, struct vc_desc *desc)
{
	...
	ctrl->exposure =
		(vc_control){ .min = 146, .max = 595000, .def = 10000 };
	AGAIN_FRA(255, 12000)

	ctrl->csr.sen.h_end = (vc_csr2){ .l = 0x0000, .m = 0x0000 };
	ctrl->csr.sen.v_end = (vc_csr2){ .l = 0x0000, .m = 0x0000 };
	ctrl->csr.sen.flash_duration =
		(vc_csr4){ .l = 0x3928, .m = 0x3927, .h = 0x3926, .u = 0x3925 };
	ctrl->csr.sen.flash_offset =
		(vc_csr4){ .l = 0x3924, .m = 0x3923, .h = 0x3922, .u = 0x0000 };
	ctrl->csr.sen.vmax =
		(vc_csr4){ .l = 0x380f, .m = 0x380e, .h = 0x0000, .u = 0x0000 };
	// NOTE: Modules rom table contains swapped address assigment.
...
int vc_mod_ctrl_init(struct vc_ctrl *ctrl, struct vc_desc *desc)
{
	...
	switch (desc->mod_id) {
	case MOD_ID_IMX178:
		vc_init_ctrl_imx178(ctrl, desc);
		break;
	case MOD_ID_IMX183:
		vc_init_ctrl_imx183(ctrl, desc);
		break;
	case MOD_ID_IMX226:
		vc_init_ctrl_imx226(ctrl, desc);
		break;
	case MOD_ID_IMX250:
		vc_init_ctrl_imx250(ctrl, desc);
		break;
	case MOD_ID_IMX252:
		vc_init_ctrl_imx252(ctrl, desc);
		break;
	case MOD_ID_IMX264:
		vc_init_ctrl_imx264(ctrl, desc);
		break;
	case MOD_ID_IMX265:
		vc_init_ctrl_imx265(ctrl, desc);
		break;
	case MOD_ID_IMX273:
		vc_init_ctrl_imx273(ctrl, desc);
		break;
	case MOD_ID_IMX290:
		vc_init_ctrl_imx290(ctrl, desc);
		break;
	case MOD_ID_IMX296:
		vc_init_ctrl_imx296(ctrl, desc);
		break;
	case MOD_ID_IMX297:
		vc_init_ctrl_imx297(ctrl, desc);
		break;
	case MOD_ID_IMX327:
		vc_init_ctrl_imx327(ctrl, desc);
		break;
	case MOD_ID_IMX335:
		vc_init_ctrl_imx335(ctrl, desc);
		break;
	case MOD_ID_IMX392:
		vc_init_ctrl_imx392(ctrl, desc);
		break;
	case MOD_ID_IMX412:
		vc_init_ctrl_imx412(ctrl, desc);
		break;
	case MOD_ID_IMX415:
		vc_init_ctrl_imx415(ctrl, desc);
		break;
	case MOD_ID_IMX462:
		vc_init_ctrl_imx462(ctrl, desc);
		break;
	case MOD_ID_IMX565:
		vc_init_ctrl_imx565(ctrl, desc);
		break;
	case MOD_ID_IMX566:
		vc_init_ctrl_imx566(ctrl, desc);
		break;
	case MOD_ID_IMX567:
		vc_init_ctrl_imx567(ctrl, desc);
		break;
	case MOD_ID_IMX568:
		vc_init_ctrl_imx568(ctrl, desc);
		break;
	case MOD_ID_IMX900:
		vc_init_ctrl_imx900(ctrl, desc);
		break;
	case MOD_ID_OV7251:
		vc_init_ctrl_ov7251(ctrl, desc);
		break;
	case MOD_ID_OV9281:
		vc_init_ctrl_ov9281(ctrl, desc);
		break;
	default:
